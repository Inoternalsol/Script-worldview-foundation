  ...
  return (
    <section className="overflow-hidden border-y border-black/5 bg-white py-10">
      ...
      <div className="relative flex w-full flex-col items-center overflow-hidden">
        {/* CSS Marquee */}
        <div className="flex w-[200%] animate-[marquee_20s_linear_infinite] md:w-full md:animate-none md:flex-wrap md:justify-center md:gap-12">
          <div className="flex w-1/2 justify-around md:w-auto md:justify-center md:gap-12">
            {partners.map((partner, idx) => (
              <div 
                key={`p1-${idx}`} 
                className="flex items-center justify-center font-heading text-xl font-bold text-gray-400 grayscale transition-all hover:text-brand-primary hover:grayscale-0"
              >
                {partner}
              </div>
            ))}
          </div>
          <div className="flex w-1/2 justify-around md:hidden">
            {partners.map((partner, idx) => (
              <div 
                key={`p2-${idx}`} 
                className="flex items-center justify-center font-heading text-xl font-bold text-gray-400 grayscale transition-all hover:text-brand-primary hover:grayscale-0"
