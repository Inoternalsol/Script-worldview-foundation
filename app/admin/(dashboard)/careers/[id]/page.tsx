...
export default function EditJobPostingPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.hash === '#applicants') {
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-brand-muted">Manage job posting details and candidate applications.</p>
        </div>
        ...
        <div className="flex rounded-lg bg-muted p-1 gap-1">
          ...
          <button
            ...
            onClick={() => {
              setActiveTab('applicants')
              window.history.pushState(null, '', `#applicants`)
            }}
            className={`flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition-all ${
      ...
      {activeTab === 'details' ? (
        ...
        <JobApplicationsTable jobId={job.id} jobTitle={job.title} />
      )}
    </div>
  )
}
